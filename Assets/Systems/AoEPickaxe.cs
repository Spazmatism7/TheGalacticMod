...
{
    public class AoEPickaxe : GlobalItem
    {
    ...
    public class SpecialPick : ModPlayer
    {
        public override void PostItemCheck()
        {
            if (!Player.inventory[Player.selectedItem].IsAir)
            {
                ...
                if (flag18)
                {
                    if (item.GetGlobalItem<AoEPickaxe>().miningRadius > 0)
                    {
                        ...
                        if (Player.toolTime == 0 && Player.itemAnimation > 0 && Player.controlUseItem)
                        {
                            ...
                            if (item.pick > 0)
                            {
                                for (int i = -item.GetGlobalItem<AoEPickaxe>().miningRadius; i <= item.GetGlobalItem<AoEPickaxe>().miningRadius; i++)
                                {
                                    for (int j = -item.GetGlobalItem<AoEPickaxe>().miningRadius; j <= item.GetGlobalItem<AoEPickaxe>().miningRadius; j++)
                                    {
