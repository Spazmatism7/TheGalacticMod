    ...
    public class MythicalWyvernHead : ModNPC 
	{
		...
		public override void SetBestiary (BestiaryDatabase database, BestiaryEntry bestiaryEntry) 
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement [] {
				...
				new FlavorTextBestiaryInfoElement("A colossal dragon-like creature unleashed along with the ancient spirits. It rules the skies with its unrivaled might.")
			});
		}
		...
		{
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<RedEnvelope>(), 1, 5, 12));
			npcLoot.Add(ItemDropRule.Common(ItemID.SoulofFlight, 1, 5, 20));
            npcLoot.Add(ItemDropRule.Common(ItemID.ReleaseLantern, 1, 1, 5));
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<MythicalLionMask>(), 10));
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<MythicalRobe>(), 10));
        }

        public override void OnKill()
        {
            Downeds.downedMythicalWyvern = true;
        }

        public override float SpawnChance(NPCSpawnInfo spawnInfo) 
		{
			if (Main.hardMode && LanternNight.LanternsUp)
