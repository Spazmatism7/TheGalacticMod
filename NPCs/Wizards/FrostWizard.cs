...
{
	public class FrostWizard : ModNPC
	{
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("A Wizard practiced in the arts of Ice")
			});
		}
		...
		public override void AI()
		{
			...
			if (NPC.ai[2] != 0f && NPC.ai[3] != 0f)
			{
				...
				{
					int num75 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y), NPC.width, NPC.height, DustID.BlueFairy, 0f, 0f, 100, default(Color), 2.5f);
				...
				{
					int num81 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y), NPC.width, NPC.height, DustID.BlueFairy, 0f, 0f, 100, default(Color), 2.5f);
			...
			if (NPC.ai[1] > 0f)
			{
				...
				if (NPC.ai[1] == 25f)
				{
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						...
						num112 *= num113;
						int num114 = 40;
						int num115 = ModContent.ProjectileType<BadIcicle>();
			...
			for (int num120 = 0; num120 < num119; num120++)
			{
				...
				{
					int num121 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y + 2f), NPC.width, NPC.height, DustID.BlueFairy, NPC.velocity.X * 0.2f, NPC.velocity.Y * 0.2f, 100, default(Color), 1.2f);
			...
			{
				int num125 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y + 2f), NPC.width, NPC.height, DustID.BlueFairy, NPC.velocity.X * 0.2f, NPC.velocity.Y * 0.2f, 100, default(Color), 2f);
		...
		{
			if (Main.hardMode && Main.player[Player.FindClosest(NPC.position, NPC.width, NPC.height)].ZoneSnow)
			{
		...
		{
			npcLoot.Add(ItemDropRule.Common(ItemType<IcicleStorm>(), 10));
		}
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            {
                Gore.NewGore(NPC.GetSource_Death(), NPC.Center, NPC.velocity, Mod.Find<ModGore>("Gore_11").Type, 1f);
                for (int i = 0; i < 10; i++)
                {
